...
def generate_new_question(mode="initial", prev_question=""):
    with st.spinner("자료를 분석하여 질문을 만들고 있습니다..."):
        ...
        * 질문 조건 준수: 질문에 "단, ~은 생략한다"와 같은 제한 조건이 있다면, 힌트와 핵심어에도 반드시 그 조건을 완벽하게 적용하여 배제한다.
        * 힌트: 질문 조건에 맞는 완성된 모범 답안을 먼저 생각한 뒤, 그 문장에서 주요 명사 및 핵심 단어 4~6개를 반드시 밑줄(______)로 완벽하게 교체하여 출력한다. 정답 단어가 힌트 문장 안에 그대로 노출되어서는 절대 안 된다.
          - 잘못된 예시: 기억세포가 활성화되어 형질세포로 분화하기 때문이다.
          - 올바른 예시: _______가 활성화되어 _______로 분화하기 때문이다.
        * 정답 노출 방지: 빈칸으로 만든 단어가 힌트의 다른 문장에 다시 등장하여 정답을 유추할 수 있게 만들면 절대 안 된다. 반복되는 단어는 모두 빈칸 처리하거나 대명사(그것, 이 물질 등)로 교체한다.
        * 핵심어 목록: 모범 답안에 포함되어야 할 빈칸의 정답 단어들을 배열 형태로 제공한다.
        ...
        {{
            ...
            "hint": "_______가 발생하여 _______에 영향을 미치기 때문이다. (이처럼 정답 단어가 완벽히 삭제되고 밑줄만 남은 문장)",
