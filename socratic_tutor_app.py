...
if "qa_history" not in st.session_state:
    st.session_state.qa_history = [] # 문제 풀이 기록 저장소
if "evaluation_done" not in st.session_state:
    st.session_state.evaluation_done = False # 평가 완료 여부
...
def generate_new_question(mode="initial", prev_question=""):
    with st.spinner("자료를 분석하여 질문을 만들고 있습니다..."):
        ...
        * 질문 조건 준수: 질문에 "단, ~은 생략한다"와 같은 제한 조건이 있다면, 힌트와 키워드에도 반드시 그 조건을 완벽하게 적용하여 배제한다.
        ...
        if user_answer:
            ...
            with st.chat_message("assistant"):
                with st.spinner("답변을 분석하여 평가를 작성 중입니다..."):
                    ...
                    * 평가 기준: 학생의 답변을 주의 깊게 읽고, 제시된 핵심 키워드가 모두 문맥에 맞게 포함되어 있다면 '정답'으로 처리한다. 학생이 키워드를 분명히 적었음에도 누락되었다고 잘못 평가하는 일이 없도록 철저히 대조한다.
                    * 두 번째 줄부터 피드백을 시작한다. 학생이 잘 파악한 키워드는 확인해준다.
                    * 학생이 빠뜨린 키워드나 논리적 오류를 교정해주며, 추가적인 질문은 하지 않고 피드백을 깔끔하게 마무리한다.
                    * 친절한 평서문 말투로 작성한다.
                    """
